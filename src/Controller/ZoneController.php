...
class ZoneController extends AbstractController
{
    ...
    if ($this->isCsrfTokenValid('delete'.$zone->getId(), $request->request->get('_token'))) {
        // Récupère les produits liés à la zone
        $produits = $zone->getProduits();

        if (count($produits) > 0) {
            // Si vous voulez supprimer tous les produits liés à cette zone
            foreach ($produits as $produit) {
                $entityManager->remove($produit);
            }
        }

        // Suppression de la zone après avoir géré les produits liés
        $entityManager->remove($zone);
        $entityManager->flush();

        $this->addFlash('success', 'La zone et tous les produits associés ont été supprimés.');
    } else {
        $this->addFlash('error', 'Token de sécurité invalide, impossible de supprimer la zone.');
    }
